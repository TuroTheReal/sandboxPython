...
| 2026-08-24 | #3 Longest Substring      | sliding_window | ~     | non    | [ ] 08-26 | [ ] 08-31 | Sliding window (fenêtre liste + `while c in window: pop(0)` + accumulateur `max_len`). Version O(n²), guidé. A bien pigé le "while pour retirer jusqu'au doublon inclus" et su lire l'erreur `pop(longest[0])`. Opti O(n) (set + left pointer) à faire. |
