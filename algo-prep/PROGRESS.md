...
| 2026-08-26 | #219 Contains Duplicate II | arrays_hashing | ~     | ~oui   | [ ] 08-28 | [ ] 09-02 | dict `{valeur: dernière position}` (réflexe Two Sum) + check `i - seen[num] <= k`. A reconnu le pattern seul, 2 fixes (sens de la distance, update inconditionnel hors `else`). Time O(n), Space O(n). |
